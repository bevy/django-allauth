...
class OAuth2Adapter(object):
    ...
    def get_access_token_data(self, request, app, client):
        print(f"get_access_token_data called with request: {request} , app: {app} , client: {client}")
        code = get_request_param(self.request, "code")
        pkce_code_verifier = request.session.pop("pkce_code_verifier", None)
        print(f"calling client.get_access_token with code: {code} , pkce_code_verifier: {pkce_code_verifier}")
...
class OAuth2CallbackView(OAuth2View):
    def dispatch(self, request, *args, **kwargs):
        print(f"OAuth2CallbackView.dispatch called with request: {request.__dict__}")
        ...
        try:
            print(f"calling adapter.get_access_token_data with request: {request.__dict__} , app: {app} , client: {client}")
            access_token = self.adapter.get_access_token_data(request, app, client)
            print(f"adapter.get_access_token_data returned access_token: {access_token}")
            token = self.adapter.parse_token(access_token)
            print(f"adapter.parse_token returned token: {token}")
            token.app = app
            print(f"calling adapter.complete_login with request: {request} , app: {app} , token: {token} , response: {access_token}")
